        ...
        <div class="col-12 col-sm-10 col-md-8 col-xl-6 mx-auto">
            ...
            <div class="card" style="width: 100%">
                ...
                <div class="signin">
                    ...
                    <form class="form-signin justify-content-center " method="POST" action="/register">
                        {{ csrf_field() }}
                            ...
                            <div class="form-group{{ $errors->has('idGrupa') ? ' has-error' : '' }}">
                                <label for="idGrupa" class="col-md-4 control-label">Grupa</label>

                                <div class="col-12">

                                    <select name="idGrupa" class="form-control" value="{{ old('idGrupa') }}" required>
                                    @foreach($grupy as $grupa)
                                                <option>{{$grupa->id}}</option>
                                            @endforeach
                                    </select>

                                    @if ($errors->has('idGrupa'))
                                        <span class="help-block">
                                        <strong>{{ $errors->first('idGrupa') }}</strong>
                                    </span>
                                    @endif
                                </div>
                            </div>
